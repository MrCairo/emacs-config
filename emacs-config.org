...
- [[#main-init-file][Main init file]]
  - [[#lispy-header-for-main-init][Lispy Header For Main Init]]
  ...
  - [[#emacs-configuration-directories][Emacs Configuration Directories]]
    - [[#files-api][Files API]]
    - [[#dirs][Dirs]]
    - [[#validate-selected-fonts][Validate Selected Fonts]]
...
* Main init file
:properties:
:custom_id: main-init-file
:visibility: children
:end:

** Lispy Header For Main Init
:properties:
:custom_id: lispy-header-for-main-init
:visibility: folded
:end:
...
These values toggle the availability of specific packages. These options are not grouped together as can be done with the =mifi-config-features= group so are all separate values.
...
:properties:
:custom_id: emacs-configuration-directories
:visibility: folded
:end:

By default, the =user-emacs-directory= points to the .emacs.d* directory from which the =init.el= is used when Emacs starts. What this means is that any package that writes to this directory will be writing files to this initialization directory. Since we want to keep this directory clean, we set this directory to something external. A new variable, =emacs-config-directory= is set to now point to the starting Emacs configuration directory.
...
**** Setup user-emacs and working-files directories for use
...
:properties:
:custom_id: validate-selected-fonts
:visibility: folded
:END:
...
:properties:
:custom_id: consult-for-org-roam
:visibility: folded
:end:
...
***** Files to open externally
:properties:
:custom_id: files-to-open-externally
:visibility: folded
:end:
  ...
  (a.k.a user-emacs-directory)."
    ...
    (let ((agenda-dir (concat org-directory "/" custom-org-agenda-directory)))
      ...
      (setq org-agenda-files (list
                               (concat agenda-dir "/mifi-journal.org")
                               (concat agenda-dir "/todos-and-follow-ups.org"))
           )))
...
|---------------------------------------------------------|
| #+auto_tangle:vars:calendar-longitude:calendar-latitude |
|                                                         |
|---------------------------------------------------------|
...
**** Org-roam config support
:properties:
:custom_id: org-roam-config-support
:visibility: folded
:end:
...
**** org-roam-capture-templates
:properties:
:custom_id: org-roam-capture-templates
:visibility: folded
:end:
...
:properties:
:custom_id: code-completion-and-handling
:visibility: folded
:end:
...
:properties:
:custom_id: lsp-mode-hook-function
:visibility: folded
:end:
...
:properties:
:custom_id: dap-generic-hydra-debug-functions
:visibility: folded
:end:
