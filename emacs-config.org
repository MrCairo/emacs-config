...
This is not 100% necessary since Ocaml can use =merlin= directly instead of fronting with an LSP server. But, we install it for any flexibility it may provide.
...
This puts a most of the installs mentioned above into a nice ~use-package~ form. This makes use of the ~opam-emacs-setup~ package in the ~lisp~ directory. It's just a stub but allows us to configure enacs using the =:ensure-system-package= option.
