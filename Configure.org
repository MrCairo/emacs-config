...
#+begin_src emacs-lisp
  ...
  (use-package yasnippet
     :defer t
     :config
     (use-(point)ackage yasnippet-snippets
        ;; :ensure t
        )
     (yas-global-mode t)
...
#+begin_src shell :results output silent
  ...
  pip3 install singleton-decorator
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package realgud
     :disabled
     :ensure nil) ;; Keep this around but right now we use DAP
...
#+begin_src emacs-lisp
  ...
  (defun mrf/delete-all-debug-session ()
  ...
  (if (package-installed-p 'dap-mode)
    (general-def python-mode-map
       ...
       "C-c . X"       'mrf/delete-all-debug-session
